...
namespace QuadTreeLib
{
	...
	public class ProportioanteSizeNodeDivider : IQuadTreeNodeDivider
	{
		...
		public QuadTreeNode CreateNewRoot(QuadTreeNode currentRoot)
		{
			...
			var newRoot = new QuadTreeNode(
				new SourceGrid.Range(startRow, startCol, halfCol * 2, halfRow * 2),
				currentRoot.Depth, currentRoot.QuadTree);
			
			
			newRoot.Nodes.Add(currentRoot);
			newRoot.Nodes.Add(new QuadTreeNode(SourceGrid.Range.From(
		...
		private void CreateNotProportionate(QuadTreeNode parentNode)
		{
			...
			parentNode.Nodes.Add(new QuadTreeNode(SourceGrid.Range.From(m_bounds.Start, halfRow, halfCol), Depth, QuadTree));
			parentNode.Nodes.Add(new QuadTreeNode(SourceGrid.Range.From(
				new Position(startRow + halfRow, startCol),
				halfRow, halfCol), Depth, QuadTree));
		}
	...
	public class QuadTreeNode
	{
		...
		public QuadTreeNode(SourceGrid.Range bounds)
		{
		...
		/// </summary>
		public QuadTreeNode(SourceGrid.Range bounds, int currentDepth, QuadTree quadTree)
			:this(bounds)
		{
			m_bounds = bounds;
			QuadTree = quadTree;
		...
		/// </summary>
		public SourceGrid.Range Bounds { get { return m_bounds; } }
		...
		/// </summary>
		public List<SourceGrid.Range> SubTreeContents
		{
			get
			{
				List<SourceGrid.Range> results = new List<SourceGrid.Range>();
		...
		public List<SourceGrid.Range> Contents { get { return m_contents; } }

		
		public List<SourceGrid.Range> Query(SourceGrid.Range queryArea)
		{
			return QueryInternal(queryArea, false);
		}
		
		public SourceGrid.Range? QueryFirst(SourceGrid.Range queryArea)
		{
		...
		/// </summary>
		/// <returns></returns>
		public List<SourceGrid.Range> QueryInternal(SourceGrid.Range queryArea, bool stopOnFirst)
		{
		...
		public List<SourceGrid.Range> Query(Position queryArea)
		{
			// create a list of the items that are found
			List<SourceGrid.Range> results = new List<SourceGrid.Range>();
		...
		public SourceGrid.Range? QueryFirst(Position queryArea)
		{
			// this quad contains items that are not entirely contained by
			// it's four sub-quads. Iterate through the items in this quad
			// to see if they intersect.
		...
		public bool Remove(SourceGrid.Range range)
		{
		...
		public void Insert(SourceGrid.Range item)
		{
			// if the item is not contained in this quad, there's a problem
